...
int main(int argc, char** argv)
{
	...
	Mat contour_mask = Mat::zeros(frame.rows, frame.cols, CV_8UC1), prevframebgrclone, framebgrclone;
	Mat noshadow_img_or,noshadow_img_and,hor_img = Mat::zeros(frame.rows, frame.cols, CV_8UC1);
	Mat ver_img = Mat::zeros(frame.rows, frame.cols, CV_8UC1), framebgr(background.size(), CV_32FC1),prevframebgr(prevframe.size(), CV_32FC1);
	...
	while (!stop) {
		...
		mog(masked_frame,foreground_shadows,0.005);
